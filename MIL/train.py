...
def train():
    ...
    epochs = args.epochs
    malicious_dataset_train = torch.load(r'D:\zzl\MutiHunter\dataloader1.pth')
    benign_dataset_train = torch.load(r'D:\zzl\MutiHunter\dataloader2.pth')
    ...
    df = pd.read_csv(f'D:\\zzl\\MutiHunter\\data\\csv_data\\pypi to pypi-1.csv',names=['package','label'])
    ...
    val_loader = DataLoader(test_dataset, batch_size=args.batch_size,
                            num_workers=args.data_loader_worker_n, shuffle=True, pin_memory=True)
    model = MILMaxPool(bert_model=args.transformer_layer_name, hidden_dim=768)
    model.train()  # 训练模式
    ...
    for epoch in range(0, epochs):
        ...
        torch.save(model.state_dict(), rf'./MIL_model{epoch+1}.pth')
